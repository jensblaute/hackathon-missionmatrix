// Modals Controller: Add Member, Add Idea, Member Spotlight, Cloud Sync & QR
...
import { connectFirebase, isCloudConnected } from '../services/sync.js';
  ...
  async openShareModal() {
    ...
    this.overlay.innerHTML = `
      <div class="modal-dialog glass-panel animate-scale-in">
        <div class="modal-header">
          <div class="modal-title-group">
            <span class="modal-badge">COLLABORATION RADAR</span>
            <h3 class="modal-title">Live Team Sync & Sharing</h3>
          </div>
          <button class="icon-btn modal-close-btn">&times;</button>
        </div>
        ...
        <div class="modal-body scrollable">
          ...
          <div class="share-hero-box">
            ...
            <div class="share-instructions">
              <h4 class="share-subhead">Join on Phone or Laptop</h4>
              <p class="share-p">Scan this QR code with your camera or share the direct link below so all teammates can view and add themselves in real time.</p>
              
              <div class="link-copy-box">
                <input type="text" class="form-input readonly-input" value="${escapeHtml(currentUrl)}" id="share-url-input" readonly />
                <button class="btn btn-secondary btn-sm" id="copy-link-btn">Copy Link</button>
              </div>
            </div>
          </div>

          <!-- Cloud Status Banner -->
          <div class="cloud-status-card ${isCloud ? 'cloud-connected' : 'cloud-ready'}">
            <div class="cloud-icon">${isCloud ? '🟢' : '☁️'}</div>
            <div class="cloud-info">
              <div class="cloud-title">${isCloud ? 'Cloud Sync Active (Firebase)' : 'Ready for Cloud Sync'}</div>
              <div class="cloud-desc">${isCloud ? 'All changes update automatically across all devices.' : 'Connect Firebase or use LAN/local sync with zero setup.'}</div>
            </div>
          </div>

          <!-- Firebase Cloud Setup Accordion -->
          <details class="tech-accordion">
            <summary class="accordion-summary">
              <span>🔧 Connect Free Firebase Project (Optional)</span>
              <span class="acc-chevron">▾</span>
            </summary>
            <div class="accordion-content">
              <p class="acc-help">Paste your free Firebase project config object below to enable unlimited real-time sync across the internet.</p>
              <textarea id="firebase-config-input" class="form-textarea" rows="4" placeholder='{"apiKey": "AIza...", "authDomain": "...", "projectId": "..."}'>${state.firebaseConfig ? JSON.stringify(state.firebaseConfig, null, 2) : ''}</textarea>
              <div class="acc-actions">
                <button type="button" class="btn btn-primary btn-sm" id="save-firebase-btn">Connect Firebase</button>
              </div>
            </div>
          </details>
          ...
          <div class="data-tools-section">
            <h4 class="section-title">📦 Board Data & Backups</h4>
            <div class="tools-btn-row">
              <button class="btn btn-ghost btn-sm" id="export-json-btn">
                <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
              ...
              <label class="btn btn-ghost btn-sm">
                ...
                <input type="file" id="import-json-input" accept=".json" class="sr-only" />
              </label>

              <button class="btn btn-danger-ghost btn-sm" id="reset-data-btn">
    ...
    // Render QR Code
    const qrCanvas = this.overlay.querySelector('#share-qr-canvas');
    if (qrCanvas) {
    ...
    // Copy link
    this.overlay.querySelector('#copy-link-btn').addEventListener('click', () => {
      const input = this.overlay.querySelector('#share-url-input');
      input.select();
      navigator.clipboard.writeText(input.value).then(() => {
        playSuccessSound();
        const btn = this.overlay.querySelector('#copy-link-btn');
        btn.textContent = 'Copied!';
        setTimeout(() => { btn.textContent = 'Copy Link'; }, 2000);
      });
    });

    // Save Firebase with flexible parser (handles raw JSON, JS objects, or const firebaseConfig = {...})
    this.overlay.querySelector('#save-firebase-btn').addEventListener('click', async () => {
      let raw = this.overlay.querySelector('#firebase-config-input').value.trim();
      if (!raw) return;
      ...
      try {
        ...
        try {
          // 2. Extract object {...} if snippet includes const firebaseConfig = ...
          const match = raw.match(/\{[\s\S]*\}/);
          if (match) {
            // Convert JS object properties to JSON (quote unquoted keys)
          ...
          // 3. Fallback: regex extraction of standard Firebase keys
          const apiKey = (raw.match(/apiKey["']?\s*:\s*["']([^"']+)["']/i) || [])[1];
          const projectId = (raw.match(/projectId["']?\s*:\s*["']([^"']+)["']/i) || [])[1];
          const authDomain = (raw.match(/authDomain["']?\s*:\s*["']([^"']+)["']/i) || [])[1];
          const storageBucket = (raw.match(/storageBucket["']?\s*:\s*["']([^"']+)["']/i) || [])[1];
          const messagingSenderId = (raw.match(/messagingSenderId["']?\s*:\s*["']([^"']+)["']/i) || [])[1];
          const appId = (raw.match(/appId["']?\s*:\s*["']([^"']+)["']/i) || [])[1];

          if (apiKey && projectId) {
            parsed = { apiKey, projectId, authDomain, storageBucket, messagingSenderId, appId };
          }
        }
      }

      if (parsed && parsed.apiKey && parsed.projectId) {
        const res = await connectFirebase(parsed);
        if (res.success) {
          playSuccessSound();
          alert('Firebase Cloud Sync Connected successfully!');
          this.close();
        } else {
          alert('Firebase connection error: ' + res.error);
        }
      } else {
        alert('Could not parse Firebase config. Please ensure apiKey and projectId are included.');
      }
    });

    // Export JSON
    this.overlay.querySelector('#export-json-btn').addEventListener('click', () => {
      const dataStr = 'data:text/json;charset=utf-8,' + encodeURIComponent(JSON.stringify(store.getState(), null, 2));
      const downloadAnchor = document.createElement('a');
      downloadAnchor.setAttribute('href', dataStr);
      downloadAnchor.setAttribute('download', `hackathon-board-${Date.now()}.json`);
    ...
    // Import JSON
    this.overlay.querySelector('#import-json-input').addEventListener('change', (e) => {
      const file = e.target.files[0];
      if (file) {
    ...
    // Reset Data
    this.overlay.querySelector('#reset-data-btn').addEventListener('click', () => {
      if (confirm('Reset board back to standard hackathon demo data?')) {
